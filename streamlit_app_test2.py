...
def fetch_invoices_via_airbyte_sonar(bearer_token: str, source_id: str, stripe_url: str) -> List[Dict[str, Any]]:
    """
    ...
    # Fallback
    st.warning("Unable to fetch invoices from Airbyte API. Proceeding without live data.")
    return []


def simplify_invoices(invoices: List[Dict[str, Any]], limit: int = 50) -> List[Dict[str, Any]]:
    """
    Reduce invoice objects to a compact subset of fields to keep the prompt concise.
    """
    fields = ["id", "customer", "status", "amount_due", "currency", "due_date", "created", "paid", "amount_paid"]
...
if st.button("Fetch & Analyze Invoices"):
    ...
    # 1) Fetch invoices via Airbyte Sonar API (outside OpenAI)
    stripe_url = "https://api.stripe.com/v1/invoices"

    fetch_start = time.time()
    invoices = fetch_invoices_via_airbyte_sonar(AIRBYTE_BEARER_TOKEN, AIRBYTE_SOURCE_ID, stripe_url)
    ...
    if invoices:
        ...
        st.info("No invoices returned from Airbyte Embedded API; analysis will proceed with empty data.")
    ...
    with st.spinner("Analyzing invoice data with the model..."):
        # Write prompt to file before calling OpenAI
        try:
            filepath = os.path.join(os.getcwd(), "test2_prompt.txt")
            # Overwrite the file each time with the latest prompt
    ...
    # 4) Render a bar chart from the fetched invoices directly (if available)
    if invoices:
